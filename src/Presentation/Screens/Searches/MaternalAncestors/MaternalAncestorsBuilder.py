...
class MaternalAncestorsBuilder:
  ...
  def build_search_button(self, command: Callable):
    self.search_button = Button(self.button_frame, "Buscar Ancestros", command)
  ...
  def build_info_label(self):
    self.info_label = tk.Label(self.result_frame, text="Selecciona una persona y presiona 'Buscar Ancestros'", 
                              font=("", 10), fg="gray")
  ...
  def display_results(self, ancestors: List[Dict[str, Any]], selected_person_name: str):
    self.result_listbox.delete(0, tk.END)
    
    if not ancestors:
      self.result_listbox.insert(tk.END, f"No se encontraron ancestros maternos para {selected_person_name}")
      self.info_label.config(text="No hay ancestros maternos para mostrar", fg="orange")
      return
    
    for ancestor in ancestors:
      info = f"{ancestor['name']} (ID: {ancestor['id']})"
      self.result_listbox.insert(tk.END, info)
    
    count = len(ancestors)
    self.info_label.config(text=f"Se encontraron {count} ancestro(s) materno(s) de {selected_person_name}", fg="green")

  def clear_results(self):
    self.result_listbox.delete(0, tk.END)
    self.info_label.config(text="Selecciona una persona y presiona 'Buscar Ancestros'", fg="gray")
