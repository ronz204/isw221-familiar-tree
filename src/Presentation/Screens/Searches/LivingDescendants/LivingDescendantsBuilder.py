...
class LivingDescendantsBuilder:
  ...
  def build_search_button(self, command: Callable):
    self.search_button = Button(self.button_frame, "Buscar Descendientes", command)
  ...
  def build_info_label(self):
    self.info_label = tk.Label(self.result_frame, text="Selecciona una persona y presiona 'Buscar Descendientes'", 
                              font=("", 10), fg="gray")
  ...
  def display_results(self, descendants: List[Dict[str, Any]], selected_person_name: str):
    self.result_listbox.delete(0, tk.END)
    
    if not descendants:
      self.result_listbox.insert(tk.END, f"No se encontraron descendientes vivos para {selected_person_name}")
      self.info_label.config(text="No hay descendientes vivos para mostrar", fg="orange")
      return
    
    for descendant in descendants:
      info = f"{descendant['name']} (ID: {descendant['id']}) | Edad: {descendant['age']} años"
      self.result_listbox.insert(tk.END, info)
    
    count = len(descendants)
    self.info_label.config(text=f"Se encontraron {count} descendiente(s) vivo(s) de {selected_person_name}", fg="green")

  def clear_results(self):
    self.result_listbox.delete(0, tk.END)
    self.info_label.config(text="Selecciona una persona y presiona 'Buscar Descendientes'", fg="gray")
