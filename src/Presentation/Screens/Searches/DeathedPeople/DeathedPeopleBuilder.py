...
class DeathedPeopleBuilder:
  ...
  def build_title(self):
    self.title = tk.Label(self.container, text="Personas Fallecidas Jóvenes", font=("", 16, "bold"))
  ...
  def build_search_button(self, command: Callable):
    self.search_button = Button(self.button_frame, "Buscar Fallecidos Jóvenes", command)
  ...
  def build_info_label(self):
    self.info_label = tk.Label(self.result_frame, text="Presiona 'Buscar Fallecidos Jóvenes' para mostrar los resultados", 
                              font=("", 10), fg="gray")
    self.info_label.grid(row=2, column=0, sticky=tk.W, pady=(10, 0))

  def display_results(self, deathed_people: List[Dict[str, Any]]):
    self.result_listbox.delete(0, tk.END)
    
    if not deathed_people:
      self.result_listbox.insert(tk.END, "No se encontraron personas fallecidas menores de 50 años.")
      self.info_label.config(text="No hay resultados para mostrar", fg="orange")
      return
    
    for person in deathed_people:
      info = f"{person['name']} (ID: {person['cedula']}) | Edad: {person['age_at_death']} años | Fecha: {person['deathdate']}"
      self.result_listbox.insert(tk.END, info)
    
    count = len(deathed_people)
    self.info_label.config(text=f"Se encontraron {count} persona(s) fallecida(s) menor(es) de 50 años", fg="green")

  def clear_results(self):
    self.result_listbox.delete(0, tk.END)
    self.info_label.config(text="Presiona 'Buscar Fallecidos' para mostrar los resultados", fg="gray")
